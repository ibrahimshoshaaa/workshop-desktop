...
# Sync: never interpret an HTTP/auth failure as an empty Firebase node.
replace_once(
    'lib/services/sync_service.dart',
    r"Future<Map<String, dynamic>\?> _fetchNode\(String path\) async \{.*?\n  \}",
...
replace_once(
    'lib/services/sync_service.dart',
    r"Future<void> _putNode\(String path, Map<String, dynamic> data\) async \{.*?\n  \}",
    '''Future<void> _putNode(String path, Map<String, dynamic> data) async {
    final uri = await FirebaseRestAuth.withAuth(Uri.parse('$_baseUrl/$path.json'));
    final response = await http
        .put(uri, headers: {'Content-Type': 'application/json'}, body: jsonEncode(data))
        .timeout(_timeout);
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw StateError('Firebase PUT failed ($path): HTTP ${response.statusCode}');
    }
  }''',
    flags=re.S,
)
...
replace_once(
    'lib/services/sync_service.dart',
    r"Future<void> _deleteNode\(String path\) async \{.*?\n  \}",
    '''Future<void> _deleteNode(String path) async {
    final uri = await FirebaseRestAuth.withAuth(Uri.parse('$_baseUrl/$path.json'));
    final response = await http.delete(uri).timeout(_timeout);
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw StateError('Firebase DELETE failed ($path): HTTP ${response.statusCode}');
    }
  }''',
    flags=re.S,
)

# Payments/expenses: conflict timestamps must use updatedAt.
replace_once(
    'lib/services/sync_service.dart',
    r"final remoteUpdatedAt = \(map\['paymentDate'\] as num\?\)\?\.toInt\(\) \?\? 0;",
    "final remoteUpdatedAt = (map['updatedAt'] as num?)?.toInt() ?? (map['paymentDate'] as num?)?.toInt() ?? 0;",
)
replace_once(
    'lib/services/sync_service.dart',
    r"'paymentDate': row\.paymentDate,\n          'paymentType': row\.paymentType,",
    "'paymentDate': row.paymentDate,\n          'updatedAt': row.updatedAt,\n          'paymentType': row.paymentType,",
)
replace_once(
    'lib/services/sync_service.dart',
    r"final remoteUpdatedAt = \(map\['date'\] as num\?\)\?\.toInt\(\) \?\? 0;",
    "final remoteUpdatedAt = (map['updatedAt'] as num?)?.toInt() ?? (map['date'] as num?)?.toInt() ?? 0;",
)
replace_once(
    'lib/services/sync_service.dart',
    r"'orderAllocationsJson': row\.orderAllocationsJson,\n          'date': row\.date,",
    "'orderAllocationsJson': row.orderAllocationsJson,\n          'date': row.date,\n          'updatedAt': row.updatedAt,",
)

# Missing permission keys are denied rather than implicitly granted.
replace_once(
    'lib/providers/auth_provider.dart',
    r"bool can\(String screenKey\) => isAdmin \|\| \(permissions\[screenKey\] \?\? true\);",
    "bool can(String screenKey) => isAdmin || (permissions[screenKey] ?? false);",
)
replace_once(
    'lib/models/app_user_model.dart',
    r"bool canAccess\(String screenKey\) => permissions\[screenKey\] \?\? true;",
    "bool canAccess(String screenKey) => permissions[screenKey] ?? false;",
)

# Do not turn an app_users request failure into an empty account list.
replace_once(
    'lib/services/user_account_service.dart',
    r"if \(response\.statusCode != 200\) return \[\];\n    final decoded = jsonDecode\(response\.body\);\n    if \(decoded is! Map\) return \[\];",
    "if (response.statusCode != 200) {\n      throw StateError('Firebase GET failed (app_users): HTTP ${response.statusCode}');\n    }\n    final decoded = jsonDecode(response.body);\n    if (decoded == null) return [];\n    if (decoded is! Map) {\n      throw StateError('Firebase returned invalid app_users data');\n    }",
)
replace_once(
    'lib/services/user_account_service.dart',
    r"final decoded = jsonDecode\(response\.body\) as Map<String, dynamic>;\n    return decoded\['name'\] as String;",
...
replace_once(
    'lib/services/user_account_service.dart',
    r"await http\.patch\(uri, body: jsonEncode\(\{'permissions': permissions\}\)\)\.timeout\(_timeout\);",
    "final response = await http.patch(uri, body: jsonEncode({'permissions': permissions})).timeout(_timeout);\n    if (response.statusCode < 200 || response.statusCode >= 300) {\n      throw StateError('فشل تحديث صلاحيات الحساب: HTTP ${response.statusCode}');\n    }",
)
replace_once(
    'lib/services/user_account_service.dart',
    r"await http\.delete\(uri\)\.timeout\(_timeout\);",
    "final response = await http.delete(uri).timeout(_timeout);\n    if (response.statusCode < 200 || response.statusCode >= 300) {\n      throw StateError('فشل حذف الحساب: HTTP ${response.statusCode}');\n    }",
)

# A deleted worker account loses its local session after a successful refresh.
replace_once(
    'lib/providers/auth_provider.dart',
...
# Archive is admin-only; archived labels use isArchived, not isDeleted.
...
if marker not in text:
    raise SystemExit('customers_screen.dart: customers provider marker not found')
...
new_button = "if (session?.isAdmin == true) IconButton(tooltip: 'أرشفة', icon: const Icon(Icons.archive_outlined, color: AppColors.danger), onPressed: () => _archive(c)),"
if old_button in text:
    text = text.replace(old_button, new_button, 1)
old_flag = "final archivedOrder = o.isDeleted;"
if old_flag in text:
    text = text.replace(old_flag, "final archivedOrder = o.isArchived;", 1)
