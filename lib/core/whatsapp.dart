...
String? normalizeEgyptianPhoneForWhatsApp(String phone) {
  ...
  if (digits.startsWith('0020'))
    digits = digits.substring(2); // 0020xxxxxxxxxx -> 20xxxxxxxxxx
  if (digits.startsWith('20') && digits.length >= 12) return digits;
  if (digits.startsWith('0'))
    digits = digits.substring(1); // 01012345678 -> 1012345678
...
Future<bool> shareTextOnWhatsApp(String text, {String? phone}) async {
  final normalized =
      phone != null ? normalizeEgyptianPhoneForWhatsApp(phone) : null;
  final encodedText = Uri.encodeComponent(text);
  final url = normalized != null
