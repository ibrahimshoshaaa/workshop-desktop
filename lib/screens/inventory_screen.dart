...
class _InventoryScreenState extends ConsumerState<InventoryScreen> {
  ...
  Future<void> _showMaterialDialog(BuildContext context, WidgetRef ref,
      {MaterialItem? material}) async {
    final formKey = GlobalKey<FormState>();
    final nameController = TextEditingController(text: material?.name ?? '');
    String unit = material?.unit ?? _units.first;
    final quantityController = TextEditingController(
        text: material?.quantity.toStringAsFixed(1) ?? '');
    final minController = TextEditingController(
        text: material?.minThreshold.toStringAsFixed(1) ?? '');
              ...
              onPressed: () async {
                ...
                if (unit.trim().isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('اكتب الوحدة')));
                  return;
                }
  ...
  Future<void> _showAdjustDialog(
      BuildContext context, WidgetRef ref, MaterialItem item) async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
                'الكمية الحالية: ${item.quantity.toStringAsFixed(1)} ${item.unit}',
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await ref
                  ...
                  .adjustMaterialQuantity(item, delta);
              if (context.mounted) Navigator.pop(context);
            },
            ...
            child: materialsAsync.when(
              data: (materials) {
                if (materials.isEmpty)
                  return const Center(
                      child: Text('لا توجد خامات مسجلة بعد',
                          style: TextStyle(color: Colors.grey)));
                final q = normalizeForSearch(_query);
                final filtered = q.isEmpty
                    ? materials
                    : materials
                        .where((m) => normalizeForSearch(m.name).contains(q))
                        .toList();
                if (filtered.isEmpty)
                  return const Center(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        onLongPress: () =>
                            _showMaterialDialog(context, ref, material: m),
                        leading: CircleAvatar(
                            backgroundColor:
                                (isLow ? AppColors.danger : AppColors.wood)
                        ...
                        subtitle: Text(
                            'الحد الأدنى: ${m.minThreshold.toStringAsFixed(1)} ${m.unit}'),
                        trailing: Column(
