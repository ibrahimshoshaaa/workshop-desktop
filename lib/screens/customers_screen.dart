...
class _CustomersScreenState extends ConsumerState<CustomersScreen> {
  ...
  Future<void> _showCustomerDialog(BuildContext context, WidgetRef ref,
  ...
  Future<void> _archive(Customer customer) async {
    ...
    final reason =
        await CustomerArchiveService(db).getArchiveBlockReason(customer.id);
    if (!mounted) return;
    if (reason != null) {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
            'سيتم نقل "${customer.name}" وكل طلباته المكتملة إلى الأرشيف. هل تريد المتابعة؟'),
        actions: [
          TextButton(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('تمت أرشفة العميل مع $count طلب.')));
  }
              ...
              builder: (context, c) => Row(children: [
                    Expanded(
                        child: Column(
                          ...
                          Text('إدارة بيانات العملاء وسجل طلباتهم',
                              style: GoogleFonts.cairo(
                  ...
                  return Padding(
                    ...
                    child: Card(
                      ...
                      child: ListTile(
                        onTap: () => showDialog(
                            context: context,
                            builder: (_) => CustomerOrdersDialog(customer: c)),
...
class CustomerOrdersDialog extends ConsumerWidget {
  ...
  const CustomerOrdersDialog(
      {super.key, required this.customer, this.includeArchived = false});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final archived = includeArchived
        ...
        : const <Order>[];
    final customerOrders = [...active, ...archived]
      ...
      content: SizedBox(
        ...
        child: customerOrders.isEmpty
            ? const _EmptyState(
                ...
                text: 'لا توجد طلبات لهذا العميل')
            : ListView.builder(
                itemCount: customerOrders.length,
                itemBuilder: (context, index) {
                  final o = customerOrders[index];
                  final archivedOrder = o.isArchived;
                  return Padding(
                    ...
                    child: Card(
                      child: ListTile(
                        ...
                        trailing: Text(
                            o.remaining > 0
                                ? 'متبقي ${o.remaining.toStringAsFixed(0)}'
                                : 'مكتمل',
                            style: TextStyle(
                                color: o.remaining > 0
                                    ? AppColors.danger
      ...
      actions: [
        ...
        if (!includeArchived)
          ElevatedButton.icon(
              ...
              onPressed: () =>
                  showAddOrderDialog(context, ref, presetCustomer: customer)),
      ],
    );
  }
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
        ...
        Text(text, style: GoogleFonts.cairo(color: Colors.grey.shade400))
      ]));
}
