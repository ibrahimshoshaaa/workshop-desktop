...
class _WorkersScreenState extends ConsumerState<WorkersScreen> {
  ...
  Future<void> _showWorkerDialog(BuildContext context, WidgetRef ref,
      {Worker? worker}) async {
    final formKey = GlobalKey<FormState>();
    final nameController = TextEditingController(text: worker?.name ?? '');
    final jobController = TextEditingController(text: worker?.jobTitle ?? '');
    final amountController = TextEditingController(
        text: worker != null ? worker.salaryAmount.toStringAsFixed(0) : '');
                  ...
                  children: [
                    TextFormField(
                      ...
                      decoration: _fieldDecoration(
                          'اسم العامل', Icons.person_outline_rounded),
                    ...
                    TextFormField(
                      ...
                      decoration: _fieldDecoration(
                          'الوظيفة (صنايعي، محاسب، مدير سوشيال...)',
                    ...
                    DropdownButtonFormField<String>(
                      value: salaryType,
                      decoration: _fieldDecoration(
                          'نوع المرتب', Icons.calendar_view_week_rounded),
                      items: _salaryTypes.entries
                    ...
                    if (salaryType == 'weekly') ...[
                      DropdownButtonFormField<int>(
                        value: payWeekday,
                        decoration: _fieldDecoration(
                            'يوم القبض الأسبوعي', Icons.event_rounded),
                    ...
                    TextFormField(
                        ...
                        decoration: _fieldDecoration(
                            'رقم الهاتف (اختياري)', Icons.phone_outlined)),
                    const SizedBox(height: 14),
                    TextFormField(
                        controller: notesController,
                        maxLines: 2,
                        decoration: _fieldDecoration(
                            'ملاحظات (اختياري)', Icons.notes_rounded)),
                  ],
                ),
              ),
            ),
        ...
        children: [
          Padding(
            ...
            child: _PageHeader(
              ...
              badge: workersAsync.value != null
                  ? '${workersAsync.value!.length} عامل'
                  : null,
                      ...
                      ...dueToday.map((w) => Padding(
                            ...
                            child: Row(
                              children: [
                                Expanded(
                                  child: Text(
                                      '${w.name} (${w.jobTitle}) - ${w.salaryAmount.toStringAsFixed(0)} ج.م',
                                      style: GoogleFonts.cairo(fontSize: 13)),
                                ),
                                ElevatedButton(
                                  onPressed: () =>
                                      _confirmPayment(context, ref, w),
              ...
              data: (workers) {
                if (workers.isEmpty) {
                  return const _EmptyState(
                      ...
                      text: 'لا يوجد عمال بعد');
                }
                final q = normalizeForSearch(_query);
                final filtered = q.isEmpty
                    ? workers
                    : workers.where((w) {
                        return normalizeForSearch(w.name).contains(q) ||
                          ...
                          child: Row(
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () =>
                                    _showWorkerDialog(context, ref, worker: w),
                              ),
                            ],
                          ),
                        ),
  ...
  Future<void> _confirmPayment(
      BuildContext context, WidgetRef ref, Worker worker) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'هل تم صرف ${worker.salaryAmount.toStringAsFixed(0)} ج.م لـ "${worker.name}"؟'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('إلغاء')),
    ...
    if (confirm == true) {
      await ref.read(repositoryProvider).confirmWorkerPayment(
          worker: worker, amount: worker.salaryAmount, periodStart: anchor);
    }
  }
}
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(
                  '${worker.jobTitle} - ${_salaryTypes[worker.salaryType]}${worker.salaryType == 'weekly' ? ' (${_weekdays[worker.payWeekday]})' : ''}',
                  style: GoogleFonts.cairo(
                ...
                data: (payments) {
                  if (payments.isEmpty) {
                    return const _EmptyState(
                        ...
                        text: 'لسه ماتسجّلش أي قبض لهذا العامل');
                  }
                  return Column(
                    children: payments
                        .map((p) => Padding(
                              ...
                              child: _MiniRow(
                                ...
                                subtitle:
                                    DateFormat('d/M/yyyy - hh:mm a', 'ar_EG')
                                        .format(
